...
0. If you haven't already done so, setup your OpenAI API key (see [OpenAI's Quickstart tutorial](https://platform.openai.com/docs/quickstart?context=python)). Note that you need to be a paying customer to access GPT-4. Also, install the OpenAI Python library with `pip install --upgrade openai` and the `validators` package with `pip install validators`.
